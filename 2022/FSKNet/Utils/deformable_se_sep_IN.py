...
from keras.layers import Input, Dense, Reshape, multiply, BatchNormalization, GlobalAvgPool2D, Permute, \
    GlobalAveragePooling3D, GlobalAveragePooling2D, SeparableConv2D
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs):
        ...
        conv1 = Conv3D(filters=16, kernel_size=(3, 3, 7), strides=(1, 1, 7), kernel_regularizer=regularizers.l2(0.01),
                       kernel_initializer='he_normal', use_bias=False, activation='relu')(input)
        conv1 = BatchNormalization()(conv1)
        conv2 = Conv3D(filters=32, kernel_size=(3, 3, 5), strides=(1, 1, 5), kernel_regularizer=regularizers.l2(0.01),
                       kernel_initializer='he_normal', use_bias=False, activation='relu')(conv1)
        conv2 = BatchNormalization()(conv2)
        print(conv2.shape)
        conv3 = Conv3D(filters=64, kernel_size=(3, 3, 3), strides=(1, 1, 3), kernel_regularizer=regularizers.l2(0.01),
        ...
        l = SeparableConv2D(filters=64, kernel_size=(3, 3,), strides=(1, 1), kernel_initializer=regularizers.l2(0.01),
                            use_bias=False, activation='relu')(l)

        l = SeparableConv2D(filters=128, kernel_size=(3, 3,), strides=(1, 1), kernel_initializer=regularizers.l2(0.01),
                            use_bias=False, activation='relu')(l)
