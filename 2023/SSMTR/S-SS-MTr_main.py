# -*- coding: utf-8 -*-
"""
Created on Thu Jul 21 10:12:09 2022
...
from HSI_SupMaeModel import mae_vit_HSI_patch3, vit_HSI_patch3
...
for num in range(0,num_of_ex):
    ...
    if args.augment:
        ...
        untrain_dataset = HyperData((train_image, train_label), transform_train)
    else:
        untrain_dataset = TensorDataset(torch.tensor(train_image), torch.tensor(train_label))    
    ...
    for epoch in range(num_epoch):
        net.train()
        total_loss = 0
        for idx, (x,y) in enumerate(untrain_loader):                        

            x = x.cuda()
            y = y.cuda()
            rec_loss, _, _, logits = net(x, mask_ratio=mask_ratio)           
