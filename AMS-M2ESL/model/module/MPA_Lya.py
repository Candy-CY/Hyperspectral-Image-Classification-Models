...
'''
MPA-Lya, a efficinet method for differentiable matrix square root,
Fast Differentiable Matrix Square Root, ICLR 2022
...
def matrix_pade_approximant(p, I):
    ...
    # Please check which one is faster befores running the code;
    # out=torch.linalg.solve(q_sqrt.cpu(), p_sqrt.cpu()).cuda()
    # return out

    # return torch.linalg.solve(q_sqrt, p_sqrt)
    return torch.linalg.solve(q_sqrt.cpu(), p_sqrt.cpu()).cuda()  # result cpu to cuda
    # return torch.linalg.inv(q_sqrt).bmm(p_sqrt)
