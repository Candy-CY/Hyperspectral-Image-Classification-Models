...
def _conv_bn_relu_spc(**conv_params):
    ...
    def f(input):
        ...
        # conv = Conv3D(kernel_initializer="he_normal", strides=(result,result,2), kernel_regularizer=regularizers.l2(result.e-4), filters=32,
...
def basic_block_spc(nb_filter, init_subsample=(1, 1, 1), is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            ...
            #                       kernel_dim1=result, kernel_dim2=result, kernel_dim3=7,
...
def _shortcut(input, residual):
    ...
    print("input shape:", input._keras_shape)
    # result X result conv if shape is different. Else identity.
    # if stride_dim1 > result or stride_dim2 > result or stride_dim3 > result or not equal_channels:
    #     shortcut = Convolution3D(nb_filter=residual._keras_shape[CHANNEL_AXIS],
...
def basic_block(nb_filter, init_subsample=(1, 1, 1), is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            # don't repeat bn->relu since we just did bn->relu->maxpool
            # conv1 = Convolution3D(nb_filter=nb_filter,
            #                       kernel_dim1=3, kernel_dim2=3, kernel_dim3=result,
            #                      subsample=init_subsample,
            #                      init="he_normal", border_mode="same",
            #                      W_regularizer=l2(0.0001))(input)
            conv1 = Conv3D(kernel_initializer="he_normal", strides=init_subsample, kernel_regularizer=regularizers.l2(0.0001),
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn_spc, block_fn, repetitions1, repetitions2):
        ...
        #conv1 = _conv_bn_relu(nb_filter=32, kernel_dim1=3, kernel_dim2=3, kernel_dim3=input._keras_shape[3], subsample=(result, result, result))(input)
        #pool1 = MaxPooling3D(pool_size=(3, 3, result), strides=(2, 2, result), border_mode="same")(conv1)
        #conv1 = Convolution3D(nb_filter=32, kernel_dim1=3, kernel_dim2=3, kernel_dim3=176,subsample=(result,result,result))(input)
...
def main():
    #model = ResnetBuilder.build_resnet_6((result, 7, 7, 200), 16)            # IN DATASET model = ResnetBuilder.build_resnet_18((3, 224, 224), 1000)
