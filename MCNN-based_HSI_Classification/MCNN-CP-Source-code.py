...
def splitTrainTestSet(X, y, testRatio, randomState=345):
    # train_test_split是交叉验证中常用的函数，功能是从样本中随机的按比例选取 train data和 test data
...
def applyPCA(X, numComponents=64):
    ...
    #用 X来训练 PCA模型，同时返回降维后的数据,newX就是降维后的数据
...
def cov_pooling(features):
    ...
    # tensorflow中的 tile()函数是用来对张量(Tensor)进行扩展的，其特点是对当前张量内的数据进行一定规则的复制。
    centers_batch = tf.tile(centers_batch, [1, shape_f[1], 1]) # 张量扩展,最终的输出张量维度不变。
    # TensorFlow中张量减法操作 tf.subtract
    ...
    trace_t = 0.0001*tf.matrix_diag(trace_t)# tf.matrix_diag生成二维的对角矩阵
