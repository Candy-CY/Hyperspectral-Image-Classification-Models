# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 08:47:49 2022
...
from HSI_ClMaeModel import mae_vit_HSI_patch3, vit_HSI_patch3
...
net_name = args.cl_mode+'_C_SS_MTr' 
...
for num in range(0,num_of_ex):
    ...
    #######################   maevit的训练
    net = mae_vit_HSI_patch3(img_size=(windowsize,windowsize), in_chans=nband, hid_chans = args.hid_chans,embed_dim=args.encoder_dim, depth=args.encoder_depth, num_heads=args.encoder_num_heads,  mlp_ratio=args.mlp_ratio,
                              decoder_embed_dim=args.decoder_dim, decoder_depth=args.decoder_depth, decoder_num_heads=args.decoder_num_heads, global_pool = False)
    net.cuda()
    ...
    for epoch in range(num_epoch):
        ...
        for idx, (x,y) in enumerate(train_loader):                        
            ...
            x = x.cuda()
            y = y.cuda()
            rec_loss,cl_loss, _, _ = net(x, mask_ratio=mask_ratio,mode = args.cl_mode) 
            loss = rec_loss + args.cl_loss_ratio * cl_loss
            optimizer.zero_grad()                             
            loss.backward()
            optimizer.step()
...
record.record_output(OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME,
                      ...
                      '_augment_' + str(args.augment) +'_aug_scale_' + str(args.scale) + '_CLloss_ratio_' + str(args.cl_loss_ratio) + '_cl_temp_' + str(args.cl_temperature) + '_decoder_dim_' + str(args.decoder_dim) + '_decoder_depth_' + str(args.decoder_depth)+ '.txt')