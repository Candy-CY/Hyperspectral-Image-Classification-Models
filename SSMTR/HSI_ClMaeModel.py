# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 08:42:06 2022
...
def D(p, z, version='simplified'): # negative cosine similarity
    if version == 'original':
        ...
        # return torch.exp(-(p*z).sum(dim=1).mean())
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, hid_chans = 32,
                 ...
                 norm_layer=nn.LayerNorm, norm_pix_loss=False, global_pool=True):
        super().__init__()
        # --------------------------------------------------------------------------
    ...
    def forward_contrastive(self, x1, x2, mode = 'Siam', temperature = 1.0):
        ...
        if mode == 'Siam':
            ...
            raise Exception('loss type must be selected from {SimCLR, Siam}')
        return L  
    ...
    def forward(self, imgs, mask_ratio=0.2, mode = 'Siam'):
        ...
        closs = self.forward_contrastive(latent_f, latent_full, mode)
         
        loss = self.forward_loss(imgs, pred, mask)
        return loss, closs, pred, mask
