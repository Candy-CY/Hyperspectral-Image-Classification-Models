...
from NetworksBlocks import SSNet_AEAE_KSC, SSRN

parser = argparse.ArgumentParser("KSC")
...
args.save = 'KSC-test-model-{}-arch-{}-{}-lr{}'.format(args.model, args.phi, time.strftime("%Y%m%d-%H%M%S"), args.learning_rate)
...
rsample_200 = [29, 10, 10, 9, 6, 9, 4, 16, 20, 15, 16, 20, 36]
...
KSC_PATH = './datasets'
mat_data = sio.loadmat(KSC_PATH + '/KSC/KSC.mat')
data_IN = mat_data['KSC']
mat_gt = sio.loadmat(KSC_PATH + '/KSC/KSC_gt.mat')
gt_IN = mat_gt['KSC_gt']
#print (data_IN.shape)
...
DEV_SIZE = 200
VAL_SIZE = 400
TEST_SIZE = TOTAL_SIZE - TRAIN_SIZE - VAL_SIZE - DEV_SIZE
...
trainloader = DataLoader(training_set, batch_size=50, shuffle=True, num_workers=8)

validationloader = DataLoader(validation_set, batch_size=50, shuffle=False, num_workers=8)

allloader = DataLoader(all_set, batch_size=50, shuffle=False, num_workers=8)

#SAVE_PATH_ = './pretrained_models/KSC-' + str(args.model) + '/' + str(args.model) + '_sample200.pth' # args.save + '/' + str(args.model) + '_sample' + str(args.sample) + '.pth' 

# Validation Stage
if args.model == 'SSTN':
    trained_net = SSNet_AEAE_KSC()
    SAVE_PATH_ = './pretrained_models/KSC-SSTN/sstn_sample200.pth'
elif args.model == 'SSRN':
    trained_net = SSRN(num_classes=13, k=85)
    SAVE_PATH_ = './pretrained_models/KSC-SSRN/ssrn_sample200.pth'
else:
    logging.error("No such model in our zoo!")

trained_net.load_state_dict(torch.load(SAVE_PATH_))
trained_net.eval()
...
# generate HSI patches for all locations
RAW_SIZE = data_IN.shape[1] * data_IN.shape[2]
raw_data = np.zeros((RAW_SIZE, INPUT_DIMENSION_CONV, 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1))
raw_assign = indexToAssignment(range(RAW_SIZE), PATCH_LENGTH, whole_data.shape[1], whole_data.shape[2])
for i in range(len(raw_assign)):
    raw_data[i] = selectNeighboringPatch(padded_data, PATCH_LENGTH, raw_assign[i][0], raw_assign[i][1])
raw_data = torch.from_numpy(raw_data)
raw_data = raw_data.to(device=device, dtype=torch.float) 
...
with torch.no_grad():
    for i in range(512):
        outputs = trained_net(raw_data[i*614:(i+1)*614])
        _, predicted = torch.max(outputs.data, 1)
        raw_pred.append(predicted)
        
raw_pred = torch.cat(raw_pred)
raw_pred = raw_pred.cpu().numpy() + 1
...
all_pred = torch.cat(raw_pred)
all_pred = all_pred.cpu().numpy() + 1
'''


#y_pred = predVisIN(all_indices, all_pred, 512, 614)
y_pred = predVisIN(range(RAW_SIZE), raw_pred, 512, 614)

#plt.plot(x, y)
plt.imshow(y_pred)
plt.axis('off')
fig_path = './Cmaps/KSC-' + str(args.model) + '.png'
