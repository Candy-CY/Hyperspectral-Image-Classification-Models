...
from NetworksBlocks import SSNet_AEAE_IN, SSRN, SSNet_AEAE_KSC
...
args.save = 'KSC-train-model-{}-arch-{}-{}-lr{}'.format(args.model, args.phi, time.strftime("%Y%m%d-%H%M%S"), args.learning_rate)
...
best_pred = 0
SAVE_PATH1 = args.save + '/' + str(args.model) + '_sample' + str(args.sample) + str(time.time()) + '.pth' 
...
trained_net.load_state_dict(torch.load(SAVE_PATH1))
trained_net.eval()
